...
export const config = {
  // `releases` is proxied to Cloudflare R2 via a next.config rewrite and must
  // NOT be rewritten to a locale-prefixed path here.
  matcher: ['/((?!_next|api|admin|releases|.*\\..*).*)'],
}
