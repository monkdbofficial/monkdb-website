...
export default function JourneyContent() {
  ...
  const t = (((dict as Record<string, unknown>).journeyPage) ?? {}) as Record<string, string>
  const tNav = (((dict as Record<string, unknown>).nav) ?? {}) as Record<string, string>
  const POWERED_BY = [
    tNav.core1 ?? 'Unified Operational Engine',
    tNav.core2 ?? 'Real-Time Processing Engine',
    tNav.core3 ?? 'AI-Native Execution Engine',
    tNav.core4 ?? 'Decision Action Engine',
    tNav.core5 ?? 'Edge to Cloud Execution Fabric',
    tNav.core6 ?? 'Sovereignty and Trust Layer',
  ]
  ...
  return (
    <main className="min-h-screen bg-white dark:bg-[#0f1623]">
      ...
      <Navbar />
      <PageBanner
        title={t.title ?? 'Developer Journey Map'}
        subtitle={t.subtitle ?? 'A guided path to understand, build, and run intelligent systems on MonkDB.'}
      />
      ...
      >
        <div className="relative z-10 max-w-[1920px] mx-auto px-5 sm:px-8 lg:px-14 xl:px-20 2xl:px-28">
          <SectionLabel text={t.stagesLabel ?? 'Stages'} />
          <motion.h2
          ...
          >
            {t.stagesTitlePart1 ?? 'Eight stages.'}{' '}
            <span className="gradient-text-animate" style={{ fontWeight: 400 }}>
              {t.stagesTitlePart2 ?? 'One continuous path.'}
            </span>
          </motion.h2>
          <motion.p
          ...
          >
            {t.stagesIntro ?? 'Each stage maps to a concrete outcome and a next-step CTA. Use this as a teaching tool, an onboarding checklist, or a reference for production rollout.'}
          </motion.p>
      ...
      >
        ...
        <div className="relative z-10 max-w-[1920px] mx-auto px-5 sm:px-8 lg:px-14 xl:px-20 2xl:px-28">
          ...
          >
            ...
            >
              {t.poweredBy ?? 'Powered by the 6 Core Systems'}
            </span>
            <ul
            ...
            >
              {t.footer ?? 'One System. One Truth. Real-Time Intelligence. Instant Action.'}
            </p>
          </motion.div>
        </div>
      </section>
      ...
      <CTABanner
        ...
        description={t.ctaDescription ?? 'Pick a stage and go. Or talk to an engineer for a guided proof of value.'}
